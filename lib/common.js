...
function veTableDatSanIndex(data) {
	...
	html += "<thead><tr><th>#</th><th>Tên KH</th><th>SĐT</th><th>Sân</th><th>Bắt đầu</th><th>Kết thúc</th><th>Phút</th><th>Đơn giá (/phút)</th><th>Tiền</th><th>Đã thanh toán</th><th>Thanh toán</th><th>Yêu cầu hủy đặt sân</th></tr></thead>";
	var tong_tien = 0;
	var da_thanh_toan = 0;
	...
	for (var i = 0; i < data.length; i++) {
		...
		html += "<button class='btnXoaDatSanIndex btn btn-light border border-dark' datsan_id='" + data[i].datsan_id + "'><i class='fas fa-times text-danger'></i></button></center></td>";
	...
	$(".btnXoaDatSanIndex").click(function() {
		...
		if (xac_nhan) {
			var ten_kh = $('.ten_kh').text();
			var sdt = $('.sdt').text();
			var ten_san = $('.ten_san').text();
			var bat_dau = $('.bat_dau').text();
			var ket_thuc = $('.ket_thuc').text();
...
function veTableDatSan(data) {
	var html = "";
	html += "<table class='mytable' style='width:100%; text-align: center;'>";
	html += "<thead><tr><th>#</th><th>Tên KH</th><th>SĐT</th><th>Sân</th><th>Bắt đầu</th><th>Kết thúc</th><th>Phút</th><th>Đơn giá (đồng/phút)</th><th>Tiền</th><th>Đã thanh toán</th><th>Thanh toán</th><th>Yêu cầu hủy đặt sân</th></tr></thead>";
	var tong_tien = 0;
	var da_thanh_toan = 0;
	var chua_thanh_toan = 0;
	for (var i = 0; i < data.length; i++) {
		...
		html += "<button class='btnXoaDatSan btn btn-light border border-dark' datsan_id='" + data[i].datsan_id + "'><i class='fas fa-times text-danger'></i></button></center></td>";
		html += "<td><center><span>" + data[i].note + "</span></center></td>";
		html += "</tr>";
	}
