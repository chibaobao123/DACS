...
function tinhtiendatsan() {
	var dongia = parseInt($("#datsan_dongia").attr("value"));
	if (dongia == "0") {
...
function veTableDatSan(data) {
	...
	html += "<thead><tr><th>#</th><th>Tên KH</th><th>SĐT</th><th>Sân</th><th>Bắt đầu</th><th>Kết thúc</th><th>Phút</th><th>Đơn giá (/phút)</th><th>Tiền</th><th>Đã thanh toán</th><th>Thanh toán</th><th>Ghi chú</th></tr></thead>";
	var tong_tien = 0;
	var da_thanh_toan = 0;
	...
	for (var i = 0; i < data.length; i++) {
		...
		if (thanh_toan == "0") {
			html += "<td><center><button class='btnThanhToan' datsan_id='" + data[i].datsan_id + "'><i class='fas fa-check'></i></button></center></td>";
		} else {
			html += "<td><center><button disabled class='btnThanhToan' datsan_id='" + data[i].datsan_id + "'><i class='fas fa-check'></i></button></center></td>";
		}
		
		html += "<td><center><button class='btnXoaDatSan' datsan_id='" + data[i].datsan_id + "'><i class='fas fa-times'></i></button></center></td>";
		html += "</tr>";
	}
