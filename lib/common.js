...
function veTableDatSanIndex(data) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		html += "<td class='sdt_kh'>" + data[i].sdt + "</td>";
	...
	$(".btnXoaDatSanIndex").click(function() {
		...
		if (xac_nhan) {
			var sdt_kh = $('.sdt_kh').text();
			xoaDatSanIndex($(this).attr("datsan_id"), sdt_kh);
		}
...
function xoaDatSan(datsan_id,sdt_kh) {
	$.ajax({
...
function xoaDatSanIndex(datsan_id,sdt_kh) {
	$.ajax({
		url: "/quanlysanbong/api/xoadatsan.php",
		type: "POST",
		cache: false,
...
function kiemtraemail(email) {
    ...
    thongbaoloi("Bạn đã nhập một địa chỉ email hợp lệ!")
    return (false);
}
